...
///labjac bullshit what the fuck did i just do		
		else if(s.equals("faggot"))
			fx = new EntityAfterburn(w, x, y, z, 0D, 0D, 0D);
		else if(s.equals("arabe"))
			fx = new EntitySmokeBurst(w, x, y, z, 0D, 0D, 0D);
		else if(s.equals("protoFlame"))
			fx = new EntityFMFlame(w, x, y, z, 0D, 0D, 0D);
		else if(s.equals("nuclear"))
			fx = new EntityFMNuke(w, x, y, z, 0D, 0D, 0D);
		else if(s.equals("smokeShell"))
			fx = new EntitySmokeShell(w, x, y, z, 0D, 0D, 0D);
		else if(s.equals("casing"))
			fx = new EntityShellCasing(w, x, y, z, 0D, 0D, 0D);
		// end of labjac bullshit
