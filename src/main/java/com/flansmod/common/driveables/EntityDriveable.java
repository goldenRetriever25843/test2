...
//public abstract class EntityDriveable extends Entity implements IExplodeable, IEntityAdditionalSpawnData
{
		...
		if(getDriveableType().collisionDamageEnable)
		{
			if(throttle > getDriveableType().collisionDamageThrottle )
			{
				if(entity instanceof EntityLiving){
					entity.attackEntityFrom(DamageSource.cactus, throttle*getDriveableType().collisionDamageTimes);
...
//default is 0.5 so half of damage a part takes is dead crew members
	...
	public void updateKeyHeldState(int key, boolean held)
	{
	    ...
	    case 30:
	    //	System.out.println("getout in driveable");
	    	if(this.seats != null && !gtfo)
	      	((ICommandSender) this.seats[0].riddenByEntity).addChatMessage(new ChatComponentText("Passengers are now banned"));
	    	gtfo = true;
	    	break;
	    ...
	    //	System.out.println("getin in driveable");
	    	if(this.seats != null && gtfo)
	    	((ICommandSender) this.seats[0].riddenByEntity).addChatMessage(new ChatComponentText("Passengers are now allowed"));
	    	gtfo = false;
	    }
	...
	//	if(disabled) return;
		...
		if(shootPoint.rootPos instanceof PilotGun)
		{
			...
			if(gunType != null && bulletItemStack != null && bulletItemStack.getItem() instanceof ItemShootable && TeamsManager.bulletsEnabled)
			{
				...
				if(gunType.isAmmo(bullet) && !type.epicShip || type.epicShip && gunType.isAmmo(bullet) && (getDriveableData().parts.get(EnumDriveablePart.buoyancy).health)>3 && ticksFlareUsing <= 0)
				{
					...
					worldObj.spawnEntityInWorld(((ItemShootable)bulletItemStack.getItem()).getEntity(worldObj, Vector3f.add(new Vector3f(posX, posY, posZ), gunVec, null), lookVector, (EntityLivingBase)seats[0].riddenByEntity, (gunType.bulletSpread) / 2, gunType.damage, (15f),bulletItemStack.getItemDamage(), type));
		...
		else //One of the other modes
		{
			...
			case SHELL :
			{
				...
				if(TeamsManager.shellsEnabled && !type.epicShip || type.epicShip && (getDriveableData().parts.get(EnumDriveablePart.buoyancy).health)>3 && ticksFlareUsing <= 0)
				{
					int slot = -1;
					...
					if(slot != -1)
					{
						...
						//where shell comes from, my chance to kill x offset so you cant shoot through walls using "gunVecFake"
						EntityShootable bulletEntity = bulletItem.getEntity(worldObj, Vector3f.add(gunVecFake, new Vector3f(posX, posY, posZ), null), lookVector, (EntityLivingBase)seats[0].riddenByEntity , spread, damageMultiplier, (pupperinoSpeed*shellSpeed*bulletItem.type.speedMultiplier), driveableData.getStackInSlot(slot).getItemDamage(), type);
						...
						if(type.farSound != null)
				        {
				                FlansMod.packetHandler.sendToDonut(new PacketPlaySound(this.posX, this.posY,
				                		...
				                		this.posZ, FlansMod.soundRange, 200, this.dimension);
				        }
 ...
 //end of sloth snip
		...
		if(type.epicShip && (ticksFlareUsing > 0 || bow.health<= 0 && bow.maxHealth>0 ||stern.health<= 0 && stern.maxHealth>0 || leftShip.health<= 0 && leftShip.maxHealth>0 || right.health<= 0 && right.maxHealth>0 || midsection.health <= 0 && midsection.maxHealth > 0))
			throttle = 0;
        ...
        for(DriveablePart part : getDriveableData().parts.values())
        {
        	if(part.box != null)
        	{
	        	...
	        	if(worldObj.isRemote)
	        	{
		        	...
		        	//visual representation of ship repair   replace with a superior long range flak later on
		        	if(type.epicShip && ticksFlareUsing>0)
		        	{
...
//part where hitting a hitbox causes bullet penetration stop
		...
		//making it that hesh cannot go through certain things kinetic can
...
//epic ship TM sinking mk1
	...
	DriveablePart buoyancy = (getDriveableData().parts.get(EnumDriveablePart.buoyancy));
		...
		//repairing is inherently 3x slower than sinking so keep your crew alive!
...
//labjac mod
		...
		//null check damage control
				...
				{
					//end of labjac damage control null checker
			if((seat != null)&&(seats[0].riddenByEntity != null && seats[0].riddenByEntity instanceof EntityPlayer)&&type.inshallah==true || (seat != null)&&(seats[0].riddenByEntity != null && seats[0].riddenByEntity instanceof EntityPlayer)&&type.canDive==true)
			{
				//((EntityPlayer)seats[0].riddenByEntity).addPotionEffect(new PotionEffect(Potion.harm.id, 10, 5));	
