...
public class EntityPlane extends EntityDriveable
{
	...
	public boolean pressKey(int key, EntityPlayer player)
	{
		...
		case 8 : //Drop bomb    um no sweetie its carrier dismount time
		{
			
			this.mountEntity(null);
			//System.out.println("carrier key did this");
			//return super.pressKey(key, player);
			return true;
		}
		case 9 : //Shoot bullet    no sweetie its carrier landing time
		...
		if(type.carrierLandable==true || type.helipadLandable==true)
		{
			...
			for(Object obj : carrier)
			{
				...
				if((carrierSpot.riddenByEntity == null && carrierSpot.seatInfo.carrier == true) || (carrierSpot.riddenByEntity == null && carrierSpot.seatInfo.helipad == true && type.helipadLandable == true))
				{
	...
	public void onUpdate()
	{
        ...
        //no afterburner throttle settles down
        if(!this.afterBurning && type.hasAfterBurner)
        {
        	if(throttle>0.501)
        throttle -= 0.005F;
        ...
        //supersonic noise
        if(this.afterBurning && throttle>0.5 && poopooTimer==0)
        {
...
//lose airworthiness without tail
		...
		if(type.labjacFuel>15)
		{
			...
			if(throttle>0.1f&&afterBurning)
				labjacFuel -=10f*(float)throttle*type.afterBurnFuelPenalty;
			else if(throttle>0.1f && !afterBurning)
				labjacFuel -=20f*(float)throttle;
			else if(throttle>0.1f && !type.hasAfterBurner)
				labjacFuel -=10f*(float)throttle;
			
		}
		
		
		//carrier announcement message   fuck it crashes if you place vehicles too much must scrap :(
