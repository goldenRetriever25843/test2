...
public class CommandTeams extends CommandBase
{
	...
	public void processCommand(ICommandSender sender, String[] split)
	{
		...
		*/
		...
		if(split[0].equals("raiding"))
		{
			...
			sender.addChatMessage(new ChatComponentText("LabJac Raiding Expolsions are now " + (TeamsManager.raiding ? "enabled" : "disabled")));
			return;
		}
		if(split[0].equals("pacifism"))
		{
			if(split.length != 2)
			{
				sender.addChatMessage(new ChatComponentText("Incorrect Usage : Should be /teams " + split[0] + " <true/false>"));
			...
			sender.addChatMessage(new ChatComponentText("Pacifism is now " + (TeamsManager.violence ? "enabled" : "disabled")));
			return;
		}
		if(split[0].equals("bleeding"))
		{
			if(split.length != 2)
			{
				sender.addChatMessage(new ChatComponentText("Incorrect Usage : Should be /teams " + split[0] + " <true/false>"));
			...
			sender.addChatMessage(new ChatComponentText("Blood is now " + (TeamsManager.bleeding ? "enabled" : "disabled")));
			return;
		}
		if(split[0].equals("seaLevel"))
		{
			if(split.length != 2)
			{
				sender.addChatMessage(new ChatComponentText("Incorrect Usage : Should be /teams " + split[0] + " <Y coord>"));
				return;
			}
			TeamsManager.seaLevel = Integer.parseInt(split[1]);
			if(TeamsManager.seaLevel > 0 && TeamsManager.seaLevel < 256)
				sender.addChatMessage(new ChatComponentText("Sea level is now considered to be " + TeamsManager.seaLevel + " meters"));
			else sender.addChatMessage(new ChatComponentText("Enter a proper number numbnuts"));
			return;
		}
	...
	public void sendHelpInformation(ICommandSender sender, int page)
	{
		...
		case 2 :
		{
			...
			sender.addChatMessage(new ChatComponentText("/teams pacifism <true / false>"));
			sender.addChatMessage(new ChatComponentText("/teams raiding <true / false>"));
			sender.addChatMessage(new ChatComponentText("/teams seaLevel <Y coordinate>"));
			sender.addChatMessage(new ChatComponentText("/teams bleeding <true / false>"));
			break;
		}
		case 3 :
		{
