...
public class ItemTeamArmour extends ItemArmor implements ISpecialArmor, IFlanItem
{
    ...
    public void onArmorTick(World world, EntityPlayer player, ItemStack itemStack)
    {
		...
		if(itemStack.getItemDamage()==0)
			timer = type.rechargeTimer;
		if(itemStack.getItemDamage()>=0 && timer>0)
			timer--;
		if(itemStack.getItemDamage()>=0 && timer==0 && type.energyShield)
		{
			rechargeTimer++;
			if(rechargeTimer==type.rechargeDelay)
				rechargeTimer=0;
			if(rechargeTimer==1)
			super.setDamage(itemStack, itemStack.getItemDamage() - 1);
		}
		if(itemStack.getItemDamage()>=0 && timer==1 && type.energyShield)
		{
		...
		SoundTimer++;
		if(SoundTimer==type.warningSoundTimer)
			SoundTimer=0;
		
if(SoundTimer==1 && timer>1)
