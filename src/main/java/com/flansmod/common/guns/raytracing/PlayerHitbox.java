...
public class PlayerHitbox 
{
	...
	public float hitByBullet(EntityBullet bullet, float penetratingPower) 
	{
		...
		if(weapon != null && weapon.getItem() instanceof ItemGun)
		{
			if(PlayerHandler.getPlayerData(player).isBlockingLeft>0 && (bullet.rightSwing || ((ItemGun)weapon.getItem()).type.shield) && trueBodyArmorPen > ((ItemGun)weapon.getItem()).type.shieldDamageAbsorption)
				{
				PlayerHandler.getPlayerData(player).isBlockingLeft = 25;
				damageModifier *= 0;
			PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, 5, player.dimension, "parry", true);
			...
			if(((ItemGun)weapon.getItem()).rightBlock && bullet.leftSwing)
			{
				damageModifier *= 0;
			PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, 5, player.dimension, "parry", true);
			FlansMod.getPacketHandler().sendToAllAround(new PacketFlak(player.posX, player.posY, player.posZ, 25, "crit"), player.posX, player.posY, player.posZ, 5, player.dimension);
			...
			if(((ItemGun)weapon.getItem()).topBlock && bullet.topSwing)
			{
				damageModifier *= 0;
			PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, 5, player.dimension, "parry", true);
			FlansMod.getPacketHandler().sendToAllAround(new PacketFlak(player.posX, player.posY, player.posZ, 25, "crit"), player.posX, player.posY, player.posZ, 5, player.dimension);
			...
			if(((ItemGun)weapon.getItem()).bottomBlock && bullet.bottomSwing)
			{
				damageModifier *= 0;
			PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, 5, player.dimension, "parry", true);
        	FlansMod.getPacketHandler().sendToAllAround(new PacketFlak(player.posX, player.posY, player.posZ, 25, "crit"), player.posX, player.posY, player.posZ, 5, player.dimension);
		...
		case BRAIN : damageModifier *= 2.5F;
		//less damage than head because frontal lobe damage = you survive but became special ed, rear brain = basic life function control
		//labjac special armor system TM
		for(int i = 0; i < 5; i++)
		{
		ItemStack stack = player.getEquipmentInSlot(i);
	...
	if(weapon != null && weapon.getItem() instanceof ItemGun)
	{
		//blocking only stops gunswords that are blockable, but shields can stop arrows too
		if(PlayerHandler.getPlayerData(player).isBlockingLeft>0 && (bullet.rightSwing || ((ItemGun)weapon.getItem()).type.shield) && trueBodyArmorPen > ((ItemGun)weapon.getItem()).type.shieldDamageAbsorption)
		{
			PlayerHandler.getPlayerData(player).isBlockingLeft = 25;
			damageModifier *= 0;
...
case FACE : damageModifier *= 1.5F;
	...
	//end of blocking system TM
		...
		switch(type)
		{
		case BODY : case BACK :  case HEAD :  case FACE : case LEFTARM :  case RIGHTARM :  case LEGS : case RIGHTITEM : case BRAIN :	case LEFTITEM : 
		{
			...
			DamageSource damagesource = bullet.owner == null ? DamageSource.generic : bullet.getBulletDamage( (type == EnumHitboxType.HEAD || type == EnumHitboxType.BRAIN) );
