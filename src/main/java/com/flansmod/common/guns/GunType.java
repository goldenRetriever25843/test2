	...
	protected void read(String[] split, TypeFile file)
	{
		...
		try
		{
			...
			//for nu recoil
				...
				sustainedRecoilPitch = Float.parseFloat(split[1]);
			else if(split[0].equals("sustainedRecoilYaw"))
				sustainedRecoilYaw = Float.parseFloat(split[1]);
...
//	public float getSpread(ItemStack stack) before goldsloth
	...
	public float getRecoilPitch(ItemStack stack)
	{
		...
		float stackRecoil = this.recoilPitch * (rand.nextFloat()-0.1f);
		for(AttachmentType attachment : getCurrentAttachments(stack))
		{
	...
	{
		//float stackRecoil = this.recoilPitch + (rand.nextFloat() * this.rndRecoilPitchRange);
		float stackSustainedRecoil = this.sustainedRecoilPitch * (rand.nextFloat()-0.1f);
		for(AttachmentType attachment : getCurrentAttachments(stack))
		{
