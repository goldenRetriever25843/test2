...
public abstract class ShootableType extends InfoType
{
	...
	//alternate model for docked on plane   ex: rocket pod vs actual rocket
	public ModelBase launcherMesh;
	//if bullet is to be visible when hanging on plane wings
	...
	//x0 for no bleeding x2 for double leeding etc
	public float bleedMultiplier = 1.0F;
	
	
	//labjac secret armor system
	public float bodyarmorPen=500;
	public float dynamicBodyarmorPen=250;
	...
	public float classicExplosionRadius = 0F;
	...
	protected void read(String[] split, TypeFile file)
	{
		...
		try
		{
			...
			if(FMLCommonHandler.instance().getSide().isClient() && split[0].equals("launcherMesh"))
				launcherMesh = FlansMod.proxy.loadModel(split[1], shortName, ModelBase.class);
			else if(split[0].equals("LauncherSkin"))
				LauncherSkin = split[1];
			...
			//labjac
				...
				missileRadarVisible = Boolean.parseBoolean(split[1]);
			...
			{
				bodyarmorPen = Float.parseFloat(split[1]);
			}
			...
			//so bodyarmor pen can weaken over time
			else if(split[0].equals("dynamicBodyArmorPen"))
			{
				dynamicBodyarmorPen = Float.parseFloat(split[1]);
			}
			else if(split[0].equals("bleedMultiplier"))
				bleedMultiplier = Float.parseFloat(split[1]);
			...
			//visible model under wing
			else if(split[0].equals("wingVisible"))
				wingVisible = Boolean.parseBoolean(split[1].toLowerCase());
			...
			//labjac obby breaker compatibility
			else if(split[0].equals("classicExplosionRadius") || split[0].equals("classicExplosion"))
				classicExplosionRadius = Float.parseFloat(split[1]);
			...
			//labjac sword cancer auto buff
			else if(split[0].equals("SwordEnergy"))
			{
				//beginning of cancer with fuse buff and speed nerf
				fuse = 4*fuse;
				//speedMultiplier = 0.2f*speedMultiplier;
				hitBoxSize = 0.7f*hitBoxSize;
