...
public class PacketReload extends PacketBase 
{
	...
	public void handleServerSide(EntityPlayerMP playerEntity) 
	{
    	...
    	if(data != null && stack != null && stack.getItem() instanceof ItemGun)
    	{
    		...
    		if(((ItemGun)stack.getItem()).reload(stack, type, playerEntity.worldObj, playerEntity, true, left))
    		{
    			...
    			data.shootTimeRight = data.shootTimeLeft = (int)( (float)type.reloadTime * (float)((ItemGun)stack.getItem()).pouchMultiplier ) + 1;
    			if(left)
    				data.reloadingLeft = true;
    			else data.reloadingRight = true;
    			
    			if(type.sidearm)
    				data.stabTimeRight = data.stabTimeLeft = (int)( (float)type.reloadTime * (float)((ItemGun)stack.getItem()).pouchMultiplier ) + 1;
	...
	public void handleClientSide(EntityPlayer clientPlayer) 
	{
    	...
    	if(stack != null && stack.getItem() instanceof ItemGun)
    	{
			...
			//here for animations
			animations.doReload((int)( (float)type.reloadTime * (float)((ItemGun)stack.getItem()).pouchMultiplier ) + 1, pumpDelay, pumpTime, chargeDelay, chargeTime);
