...
public class GuiDriveableController extends GuiScreen
{
	...
	public void handleInput()
    {
		...
		if(plane != null && !plane.isDead() && plane.getControllingEntity() != null && plane.getControllingEntity() instanceof EntityPlayer)
		{
			...
			if(!getOut&& FlansMod.proxy.keyDown(KeyInputHandler.eject.getKeyCode())) //for kicking people out of seats
			{
				//System.out.println("getout true");
				getOut = true;
				plane.updateKeyHeldState(30, true);
			}
			if(getOut && FlansMod.proxy.keyDown(KeyInputHandler.eject.getKeyCode()))
			{
				//System.out.println("getout false");
				getOut = false;
				plane.updateKeyHeldState(30, false);
			}
			
			if(!getOut&& FlansMod.proxy.keyDown(KeyInputHandler.openBorders.getKeyCode())) //for open border seats
			{
				//System.out.println("getout true");
				getIn = true;
				plane.updateKeyHeldState(31, true);
			}
			if(getOut && FlansMod.proxy.keyDown(KeyInputHandler.openBorders.getKeyCode()))
			{
				//System.out.println("getout false");
				getIn = false;
				plane.updateKeyHeldState(31, false);
			}
