...
import com.flansmod.client.virtualreality.HardnessMultiplier;
import com.flansmod.client.virtualreality.HolyGrailOfSemen;
import com.flansmod.client.virtualreality.LibidoListener;
import com.flansmod.client.virtualreality.SexDoll;
...
public class ClientProxy extends CommonProxy
{
	...
	public void load()
	{
		...
		//if you touch anything here the game will crash on launch
        facts = "u";
        FlansMod.log(facts);
        RenderingRegistry.registerEntityRenderingHandler(HolyGrailOfSemen.class, new SexDoll());
        MinecraftForge.EVENT_BUS.register(new HardnessMultiplier());        
        FMLCommonHandler.instance().bus().register(new LibidoListener());
	}
	...
	public void doTutorialStuff(EntityPlayer player, EntityDriveable entityType)
	{
		//if (!FlansModClient.doneTutorial)
		{
			...
			player.addChatComponentMessage(new ChatComponentText("Hold " + Keyboard.getKeyName(Minecraft.getMinecraft().gameSettings.keyBindSneak.getKeyCode()) + " to get out"));
			player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.eject.getKeyCode()) + " to eject and ban passengers"));
			player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.openBorders.getKeyCode()) + " to allow passengers again"));
			//player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.modeKey.getKeyCode()) + " to switch seats"));	

			if (entityType instanceof EntityPlane)
			{
				player.addChatComponentMessage(new ChatComponentText("Hold " + Keyboard.getKeyName(KeyInputHandler.nintendoSwitch.getKeyCode()) + " to switch seats"));
				player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.controlSwitchKey.getKeyCode()) + " for mouse control mode"));
				...
				if(PlaneType.getPlane(((EntityPlane)entityType).driveableType).hasAfterBurner)
					player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.trimKey.getKeyCode()) + " to toggle afterburner"));
				...
				if(PlaneType.getPlane(((EntityPlane)entityType).driveableType).carrierLandable || PlaneType.getPlane(((EntityPlane)entityType).driveableType).helipadLandable)
					player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.gunKey.getKeyCode()) + " to dock helicopter/helicopter/floatplane"));
				if(entityType.getDriveableType().hasMagicArtilleryMode)
					player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.artilleryMode.getKeyCode()) + " to toggle magic ammo vision"));
			}
			if (entityType instanceof EntityVehicle)
			{
				...
				if(((EntityVehicle) entityType).getVehicleType().tank)
					player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.leftRollKey.getKeyCode()) + " to lock a tank turret / cannon"));
				if(((EntityVehicle) entityType).getVehicleType().tank)
					player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.rightRollKey.getKeyCode()) + " to unlock a tank turret / cannon"));
				if(((EntityVehicle) entityType).getVehicleType().thermalSight)
					player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.thermal.getKeyCode()) + " to toggle thermal sights"));
				if(entityType.getDriveableType().hasMagicArtilleryMode)
					player.addChatComponentMessage(new ChatComponentText("Press " + Keyboard.getKeyName(KeyInputHandler.artilleryMode.getKeyCode()) + " to toggle magic ammo vision"));
				player.addChatComponentMessage(new ChatComponentText("Press the spacebar to brake"));
			}
		}
	}
