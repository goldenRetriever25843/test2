			...
			case EQUIPPED_FIRST_PERSON:
			{
				...
				/* crouching ads zoom fugged
					...
					GL11.glTranslatef(model.crouchZoom, -0.03F * adsSwitch, 0F);
						...
						&& !ItemGun.shooting && model.fancyStance || PlayerHandler.getPlayerData((EntityPlayer)Minecraft.getMinecraft().thePlayer).isBlockingLeft>0)
				{
					...
					/*   before fucking with it  -gun render land that i labjac ruined, dill render world
					 * GL11.glRotatef(25F - 5F * adsSwitch, 0F, 0F, 1F);
					GL11.glRotatef(-5F, 0F, 1F, 0F);
					GL11.glTranslatef(0.15F, 0.2F + 0.175F * adsSwitch, -0.6F - 0.405F * 2 * adsSwitch);
					*/
					
					GL11.glRotatef(25F - 5F * adsSwitch, 0F, 0F, 1F);
					GL11.glRotatef(-15F, 0F, 1F, 0F); 
					GL11.glRotatef(10F * adsSwitch, 0F, 1F, 0F);
					GL11.glTranslatef(0f, 0.0275f, -0.1f);
					//the line for adjusting elevation of ads
...
//mount and blade labjac				
				if (animations.meleeLeftAnimationProgress > 0 && animations.meleeLeftAnimationProgress < gunType.meleeLeftPath.size())
				{
					...
					GL11.glRotatef(meleeLeftAngles.y + (nextMeleeAngles.y - meleeLeftAngles.y) * smoothing, 0F, 1F, 0F);
					GL11.glRotatef(meleeLeftAngles.z + (nextMeleeAngles.z - meleeLeftAngles.z) * smoothing, 0F, 0F, 1F);
					GL11.glRotatef(meleeLeftAngles.x + (nextMeleeAngles.x - meleeLeftAngles.x) * smoothing, 1F, 0F, 0F);
				}				
				else if (animations.meleeRightAnimationProgress > 0 && animations.meleeRightAnimationProgress < gunType.meleeRightPath.size())
				{
					...
					GL11.glRotatef(meleeRightAngles.y + (nextMeleeAngles.y - meleeRightAngles.y) * smoothing, 0F, 1F, 0F);
					GL11.glRotatef(meleeRightAngles.z + (nextMeleeAngles.z - meleeRightAngles.z) * smoothing, 0F, 0F, 1F);
					GL11.glRotatef(meleeRightAngles.x + (nextMeleeAngles.x - meleeRightAngles.x) * smoothing, 1F, 0F, 0F);
				}		
				else if (animations.meleeDownAnimationProgress > 0 && animations.meleeDownAnimationProgress < gunType.meleeDownPath.size())
				{
					...
					GL11.glRotatef(meleeDownAngles.y + (nextMeleeAngles.y - meleeDownAngles.y) * smoothing, 0F, 1F, 0F);
					GL11.glRotatef(meleeDownAngles.z + (nextMeleeAngles.z - meleeDownAngles.z) * smoothing, 0F, 0F, 1F);
					GL11.glRotatef(meleeDownAngles.x + (nextMeleeAngles.x - meleeDownAngles.x) * smoothing, 1F, 0F, 0F);
				}			
				//end mount and blade
	...
	public void renderGun(ItemStack item, GunType type, float f, ModelGun model, GunAnimations animations, float reloadRotate, ItemRenderType rtype)
	{
		...
		GL11.glPushMatrix();
		{
			...
			GL11.glPushMatrix();
			{
				...
				switch (anim)
				{
					case END_LOADED: case BACK_LOADED: case CROSSBOW:
					{
						if (empty)
							shouldRender = false;
						break;
				...
				if (shouldRender && animations.reloading && Minecraft.getMinecraft().gameSettings.thirdPersonView == 0)
				{
					...
					case RIFLE_TOP: case CUSTOMRIFLE_TOP:
						...
						// before ghetto fix float ammoPosition = clipPosition * getNumBulletsInReload(animations, gripAttachment, type, item); allah bookmark
						{
							float ammoPosition = clipPosition * 1;
							int bulletNum = MathHelper.floor_float(ammoPosition);
							float bulletProgress = ammoPosition - bulletNum;
		...
		GL11.glPushMatrix();
		{
			...
			if (!model.rightHandAmmo) {
				...
				//baris vr
		           for(int a = 0; a<4; a++) {
	                    //TODO BURA
	                     if(Minecraft.getMinecraft().thePlayer.getCurrentArmor(a)!=null) {
	                             if(Minecraft.getMinecraft().thePlayer.getCurrentArmor(a).getItem() instanceof ItemTeamArmour) {
	                                 ItemTeamArmour kiyafet = (ItemTeamArmour) Minecraft.getMinecraft().thePlayer.getCurrentArmor(a).getItem();
	                                 ModelCustomArmour sekil = (ModelCustomArmour) kiyafet.type.GetModel();
//	                                 System.out.println(socks.type.armourTextureName);
	                                // Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation("flansmod:armor/"+kiyafet.type.armourTextureName+"_1.png"));
