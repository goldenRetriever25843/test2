...
public class CommandClowder extends CommandBase {
	...
	public void processCommand(ICommandSender sender, String[] args) {
		...
		if(cmd.equals("suckoff") && args.length > 1) {
			cmdSuckoff(sender, args[1]);
			return;
		}
		
		if(cmd.equals("accepttribute") && args.length > 1) {
		...
		if(cmd.equals("fabricate") && args.length > 1) {
			cmdFabricate(sender, args[1]);
			return;
		}
		
		if(cmd.equals("declare")) {
	...
	private void cmdHelp(ICommandSender sender, String page) {
		...
		if(p == 1) {
			...
			sender.addChatMessage(new ChatComponentText(COMMAND_LEADER + "-rename <name>" + TITLE + " - Renames your faction for a prestige fee"));
			sender.addChatMessage(new ChatComponentText(COMMAND_LEADER + "-suckoff <name>" + TITLE + " - Sends a tribute offer to a faction"));
			sender.addChatMessage(new ChatComponentText(COMMAND_LEADER + "-accepttribute <name>" + TITLE + " - Accepts a player's tribute offer"));
			sender.addChatMessage(new ChatComponentText(INFO + "/clowder help 2"));
		}

		if(p == 2) {
			sender.addChatMessage(new ChatComponentText(COMMAND + "-fabricate <name>" + TITLE + " - Fabricates a war against a faction, costs " + MainRegistry.fabricateCost + " prestige"));
			sender.addChatMessage(new ChatComponentText(COMMAND + "-declare" + TITLE + " - Declares war against your target if you have already fabricated a war against them"));
		...
		if(p == 4) {
			sender.addChatMessage(new ChatComponentText(COMMAND + "-retreat" + TITLE + " - Protects entire faction from enemy fabrications by retreating after 10 minutes"));
			sender.addChatMessage(new ChatComponentText(COMMAND + "-retreat" + TITLE + " - Retreat wont work if the enemy declared a war, so do it quick!"));
	...
	private void cmdColor(ICommandSender sender, String color) {
		...
		if(clowder != null) {
			...
			if(clowder.getPermLevel(player.getDisplayName()) > 1) {
				...
				sender.addChatMessage(new ChatComponentText(ERROR + "You lack the permissions to change this faction's color!"));
			}
	...
	private void cmdInfo(ICommandSender sender, String name) {
		...
		if(clowder != null) {
			...
			/*
			//test ping everyone noise
			for(Clowder everyone : clowder.clowders)
			{
			//everyone.notifyAll(player.worldObj, new ChatComponentText(CommandClowder.TITLE + clowder.name + " has declared war against " + clowder.enemy.name + "!"));
			...
			victim.worldObj.playSoundEffect(victim.posX, victim.posY, victim.posZ, "hfr:item.hoiWar", 2.0F, 1.0F);
	...
	private void cmdFabricate(ICommandSender sender, String name) {
		...
		{	
			...
			else if (victim.isRaidable())
			{
				...
				{
					sender.addChatMessage(new ChatComponentText(TITLE + "War fabrication process has begun!"));
				attacker.notifyAll(player.worldObj, new ChatComponentText(CommandClowder.ERROR + "Allah be with us! We will soon vanquish " + victim.name + "! Use /c info to see remaining time!"));
			...
			//	Minecraft.getMinecraft().thePlayer.playSound("hfr:item.hoiFabrication", 0.5F, 1.0F);
				victim.notifyAll(player.worldObj, new ChatComponentText(CommandClowder.ERROR + "WARNING! " + attacker.name + " is preparing to attack you!"));
				
				//prestige price toll paid
				attacker.addPrestige(-MainRegistry.fabricateCost, player.worldObj);
				
				//for now 3 minute fabrication for testing - change to bigger number later allah bookmark
				attacker.addFabricateTime(3, player.worldObj);
				//marks your target as your faction's enemy
				attacker.enemy = victim;
				
				//mark the victim as being targeted unless they pussy out
				if (!victim.targeted && victim.isRaidable())
					victim.targeted = true;
				
				}
				
				else if (attacker.getFabricatetime() > 0 && attacker.enemy != null)
					sender.addChatMessage(new ChatComponentText(ERROR + "You are already fabricating a war! Use /c info to see the remaining time and target!"));
				else if (attacker.getWartime() > 0  && attacker.enemy != null)
					sender.addChatMessage(new ChatComponentText(ERROR + "You are already at war!"));
				else if (attacker.getCanDeclareTime() > 0  && attacker.enemy != null)
					sender.addChatMessage(new ChatComponentText(ERROR + "War declaration is already complete! Use /c declare to begin the war!"));
				
				}
					else
						sender.addChatMessage(new ChatComponentText(ERROR + "You cannot declare war against your master! You must start a revolt using (WIP)"));
				}
					else
						sender.addChatMessage(new ChatComponentText(ERROR + "You cannot declare war against your tributary! You must cancel the tribute using (WIP)"));
				}
				else
					sender.addChatMessage(new ChatComponentText(ERROR + "You do not have enough prestige ("+ MainRegistry.fabricateCost +") to fabricate a war!"));
		...
		sender.addChatMessage(new ChatComponentText(ERROR + "You lack the permissions to fabricate wars!"));
			
		}
		else
		sender.addChatMessage(new ChatComponentText(ERROR + "You need to be in a faction and have a valid target to fabricate wars!"));
	}
		...
		if(clowder != null) {
			...
			if(Clowder.getClowderFromName(name) == null) 
			{
				...
				if(clowder.getPermLevel(player.getDisplayName()) > 1) {
					...
					sender.addChatMessage(new ChatComponentText(ERROR + "You need at least " + MainRegistry.renameCost + " prestige to create a warp!"));
				}
	...
	private void cmdSuckoff(ICommandSender sender, String name) {
		...
		if(clowder != null) {
			...
			sender.addChatMessage(new ChatComponentText(ERROR + "You need to be in a faction!"));
		}
	}
			...
			if(clowder.getPermLevel(player.getDisplayName()) > 1) {
				...
				{ //checks if the name of the guy you typed in command actually applied to become your slave
					...
					if(Clowder.getClowderFromPlayerName(name) != null) 
					{
						...
						sender.addChatMessage(new ChatComponentText(ERROR + "This player is not in another faction!"));
					}
	...
	private void cmdRetreat(ICommandSender sender) {
		...
		if(clowder != null) {
			...
			if(!Clowder.retreating.contains(player.getDisplayName())) {
				...
				clowder.notifyAll(player.worldObj, new ChatComponentText(INFO + "In 10 minutes we will all be kicked and enemy war fabrications will be cancelled!"));
				clowder.notifyAll(player.worldObj, new ChatComponentText(ERROR + "Warning: This will NOT work if the enemy fully declares war!"));
				
				for(String members : clowder.members.keySet())
				Clowder.retreating.add(members);
				
			} else {
				sender.addChatMessage(new ChatComponentText(ERROR + "A retreat order has already been ordered!"));
			}
