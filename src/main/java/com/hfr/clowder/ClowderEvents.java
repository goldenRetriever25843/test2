...
public class ClowderEvents {
	...
	public void clowderExplosionEvent(Detonate event) {
		...
		if(event.explosion.exploder instanceof EntityBullet)
		{
			...
			if (Raiderino.owner != null)
			{
				...
				{
				 if (shooterFac.enemy == owner.owner && shooterFac.getWartime() <= 0 && !canExplode(owner, event.world, x, y, z) || owner.owner.enemy == shooterFac.enemy && owner.owner.getWartime() <= 0)
				{
					event.getAffectedBlocks().clear();
				bb = false;
				}
	...
	public void clowderContainerEvent(PlayerInteractEvent event) {
		...
		if(event.action == Action.RIGHT_CLICK_BLOCK) {
			...
			if(owner != null) {
				...
				//include bypass so landmines cna be placed in enemy land
	...
	//this is the void that activates when you enter a territory to display epic graphical flag! - important allah note
	private void flagPopup(World world, EntityPlayer player) {
		...
		if(!name.equals(past)) {
			...
			if(owner.zone == Zone.FACTION) {
				...
				//allah notes the brown owner means owner of flag, and the second owner is for the faction itself. xddd bob you cheeky bastard you jerry rigged the flag display code for this
	...
	public void onEntityKilled(LivingDeathEvent event)  
	{
		...
		if(entity instanceof EntityPlayer)
		{
			...
			//war ends faster if an attacker dies during a war
			if (clowder != null)
			{
				if (clowder.getWartime() > 0)
					clowder.addWarTime(-1, human.worldObj);
			}
		}	
	}
		...
		if(!player.worldObj.isRemote) {
			...
			if(clowder != null && clowder.members.get(name) != null) {
				...
				if(!Clowder.retreating.contains(name)) {
					...
					//one minute temporarily for testing
					long time = 60 * 1 * 1000;
					...
					if(l < System.currentTimeMillis()) {
						...
						if(clowder.targeted)
						{
							System.out.println("retreat caused targeted removal");
							...
							for(Clowder everyone : clowder.clowders)
							{
								if (everyone.enemy == clowder && everyone.getWartime() == 0)
								{
									...
									everyone.notifyAll(player.worldObj, new ChatComponentText(CommandClowder.TITLE + clowder.name + " has retreated, cancelling our war fabrication!"));
					...
					//	mp.playerNetServerHandler
						
						mp.playerNetServerHandler.kickPlayerFromServer("You have just retreated! If you log back in, your enemies might try to fabricate against you again!");
					}
				}
